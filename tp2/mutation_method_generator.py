import math
from src.utils.class_utils import get_attribute_sets, get_character_class, get_crossing_method, get_cutoff_method, get_mutation_method, get_population, get_replacement_method, get_selection_method, CHARACTER_TYPES, MUTATION_METHODS
from src.utils.generator_utils import BEST_CROSSING_METHOD, DEFAULT_MUTATION_RATE, DEFAULT_FIRST_SELECTION_METHOD, DEFAULT_SECOND_SELECTION_METHOD, DEFAULT_A_VALUE, DEFAULT_INDIVIDUALS, DEFAULT_CUTOFF_METHOD, DEFAULT_CUTOFF_VALUE, DEFAULT_CUTOFF_GENERATIONS, DEFAULT_REPLACEMENT_METHOD, DEFAULT_B_VALUE, DEFAULT_REPLACEMENT_FIRST_SELECTION_METHOD, DEFAULT_REPLACEMENT_SECOND_SELECTION_METHOD, DEFAULT_ATTRIBUTE_SETS
import csv
...
if __name__ == "__main__":
    ...
    with open('output/mutation_method.csv', 'a', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['Character', 'Mutation Method', 'Mutation Rate', 'Generation', 'Best Fitness', 'Average Fitness'])

    for character in CHARACTER_TYPES.keys():
        for mutation in MUTATION_METHODS.keys():
                ...
                for _ in range(50):
                    ...
                    with open('output/mutation_method.csv', 'a', newline='') as csvfile:
                        ...
                        for i, old_generation in enumerate(old_generations):
                            ...
                            writer.writerow([character, mutation, mutation_rate, i, best_fitness, average_fitness])
